...
const getFiles = (api: string): SidebarItemConfig[] => {
  return fs
    .readdirSync(`./docs/api-${api}`)
    .filter((file: string) => file.includes("_8h"))
    .map((file: string) => {
...
const getClasses = (api: string): SidebarItemConfig[] => {
  return fs
    .readdirSync(`./docs/api-${api}`)
    .filter((file: string) => file.match(/^class[^_]/))
    .map((file: string) => {
      // Keep the file name only and remove the directories
      const fileName = file.split("/").pop().replace(".md", "");
      ...
      return {
        type: "doc",
        id: `api-${api}/` + fileName,
        label: decodeFileName(fileName).replace("class", ""),
      };
    });
};
...
const getStructs = (api: string): SidebarItemConfig[] => {
  return fs
    .readdirSync(`./docs/api-${api}`)
    .filter((file: string) => file.match(/^struct[^_]/))
    .map((file: string) => {
      // Keep the file name only and remove the directories
      const fileName = file.split("/").pop().replace(".md", "");
      ...
      return {
        type: "doc",
        id: `api-${api}/` + fileName,
        label: decodeFileName(fileName).replace("struct", ""),
      };
    });
};
  ...
  if (fs.existsSync(`./docs/api-libf3d`)) {
    return {
      ...
      link: {
        ...
        description:
          "This is the automatically generated doxygen documentation for the libf3d, which is part of F3D. It contains all the classes, structures, functions, and other elements that make up the library. You can use this documentation to understand how to use the libf3d in your own applications, as well as to explore its features and capabilities.",
      },
      items: [
        {
          type: "doc",
  ...
  if (fs.existsSync(`./docs/api-vtkext`)) {
    return {
      ...
      link: {
        ...
        description:
          "This is the automatically generated doxygen documentation for the vtkext, which is part of F3D. It contains the VTK module you can use to create f3d plugins.",
      },
      items: [
        {
          type: "doc",
