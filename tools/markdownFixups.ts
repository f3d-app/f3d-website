function processUserOptions(content: string): string {
  ...
  function fixCliHeaders(_: string, ...args: string[]) {
    ...
    const flag_list = Array.from(flags_md.matchAll(/(-+[^=`]+)/g)).map(
      (m) => m[0],
    );
    const anchor = flag_list.reduce((a, b) => (a.length > b.length ? a : b));
    for (const flag of flag_list) known_flags[flag] = anchor;
  ...
  function addAnchorLinksForKnownFlags(substring: string, ...args: string[]) {
    function make_link(k: string, v: string) {
      return known_flags[k]
        ? `[\`${k}${v}\`](#${known_flags[k]})`
        : `\`${k}${v}\``;
    }
    const is_header = args[args.length - 1].startsWith("#");
    if (!is_header && args[0] != undefined)
      return make_link(args[1], args[2] || "");
    if (!is_header && args[3] != undefined)
      return make_link(args[4], args[5] || "");
    return substring;
  }

  let lines = content.split(/\r?\n/g);
  lines = lines.map((l) =>
    l.replace(/(##+) *(`[^\(\)]+) *(\(.+\))?/, fixCliHeaders),
  );
  lines = lines.map((l) =>
    l.replaceAll(
      /(`(--[^=`]+)(=[^`]*)?`)|(`(-[^=`])([^`]*)?`)/g,
      addAnchorLinksForKnownFlags,
    ),
  );
  return lines.join("\n");
}
