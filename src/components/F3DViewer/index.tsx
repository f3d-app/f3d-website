...
function initViewer(moduleRef, fileUrl, addLog, setIsLoading) {
  ...
  f3d({ canvas: canvas })
    .then(async (Module) => {
      ...
      // setup coloring
      moduleRef.current.engineInstance.getOptions().toggle("model.scivis.enable");
      moduleRef.current.engineInstance
        .getOptions()
      ...
      moduleRef.current.engineInstance.getOptions().toggle("model.scivis.cells");

      // make it look nice
      moduleRef.current.engineInstance
        .getOptions()
        .toggle("render.effect.antialiasing.enable");
      moduleRef.current.engineInstance.getOptions().toggle("render.effect.tone_mapping");
      moduleRef.current.engineInstance
        .getOptions()
        .toggle("render.effect.ambient_occlusion");
      moduleRef.current.engineInstance.getOptions().toggle("render.hdri.ambient");

      // display widgets
      moduleRef.current.engineInstance.getOptions().toggle("ui.axis");
      moduleRef.current.engineInstance.getOptions().toggle("render.grid.enable");
      ...
      // load file
      openFile(moduleRef, new Uint8Array(defaultFile));
...
function openFile(moduleRef, stream: Uint8Array) {
...
const F3DViewer = forwardRef<any, F3DViewerProps>(({ fileUrl }, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    ...
    setUpDirection: (direction) => {
      ...
      openFile(moduleRef, moduleRef.current.currentStream);
    },
